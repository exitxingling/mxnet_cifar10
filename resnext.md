...
class resnext_bottleneck(nn.HybridBlock):
    def __init__(self, channels, cardinality = 8, widen_factor = 4, 
                 same_shape=True, dim_same = True, **kwargs):
        super(resnext_bottleneck, self).__init__(**kwargs)
        self.same_shape = same_shape
        self.dim_same = dim_same
        self.D = cardinality * channels // widen_factor
        with self.name_scope():
            strides = 1 if same_shape else 2
            self.conv1 = nn.Conv2D(self.D, kernel_size=1, use_bias = False)
            self.bn1 = nn.BatchNorm()
            self.conv2 = nn.Conv2D(self.D, kernel_size=3, padding=1, groups = cardinality,
...
class ResNeXt(nn.HybridBlock):
    def __init__(self, num_classes, verbose=False, **kwargs):
        ...
        with self.name_scope():
            ...
            # block 2
            net.add(resnext_bottleneck(64 * 4, dim_same = False))
            for _ in range(2):
                net.add(resnext_bottleneck(64 * 4))
            # block 3
            net.add(resnext_bottleneck(128 * 4, same_shape=False))
            for _ in range(2):
                net.add(resnext_bottleneck(128 * 4))
            # block 4
            net.add(resnext_bottleneck(256 * 4, same_shape=False))
            for _ in range(2):
                net.add(resnext_bottleneck(256 * 4))
            # block 5
            net.add(nn.BatchNorm())
...
def train(net, train_data, valid_data, num_epochs, lr, wd, ctx, lr_period, lr_decay):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > 0 and epoch == lr_period:
            ...
            net.save_params('resnext29_16_64d%depoch.params' % epoch)
        elif epoch > 0 and (epoch - 75) == lr_period:
            trainer.set_learning_rate(trainer.learning_rate * lr_decay)
            net.save_params('resnext29_16_64d%depoch.params' % epoch)
        elif epoch > 0 and (epoch - 125) == lr_period:
            trainer.set_learning_rate(trainer.learning_rate * lr_decay)
            net.save_params('resnext29_16_64d%depoch.params' % epoch)
        elif epoch > 0 and (epoch - 150) == lr_period:
            net.save_params('resnext29_16_64d%depoch.params' % epoch)
...
[
 {
  "name": "stdout",
  "output_type": "stream",
  "text": "Epoch 0. Loss: 7.337598, Train acc 0.080000, Time 00:00:41, lr 0.1\nEpoch 1. Loss: 2.601548, Train acc 0.080000, Time 00:00:40, lr 0.1\nEpoch 2. Loss: 2.535892, Train acc 0.090000, Time 00:00:40, lr 0.1\nEpoch 3. Loss: 2.583956, Train acc 0.070000, Time 00:00:41, lr 0.1\nEpoch 4. Loss: 2.526662, Train acc 0.070000, Time 00:00:41, lr 0.1\nEpoch 5. Loss: 2.570862, Train acc 0.100000, Time 00:00:41, lr 0.1\nEpoch 6. Loss: 2.489398, Train acc 0.140000, Time 00:00:41, lr 0.1\nEpoch 7. Loss: 2.556729, Train acc 0.100000, Time 00:00:41, lr 0.1\nEpoch 8. Loss: 2.577396, Train acc 0.080000, Time 00:00:41, lr 0.1\nEpoch 9. Loss: 2.566071, Train acc 0.070000, Time 00:00:41, lr 0.1\n"
 }
]
```
